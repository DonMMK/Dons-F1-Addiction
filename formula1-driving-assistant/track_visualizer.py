...
def _draw_zones(
    ax, x: np.ndarray, y: np.ndarray, zones: DrivingZones, telemetry: TelemetryData
):
...
def _draw_speed_gradient(ax, x: np.ndarray, y: np.ndarray, speed: np.ndarray):
    ...
    cmap = LinearSegmentedColormap.from_list(
        "speed", ["#0000FF", "#00FF00", "#FFFF00", "#FF0000"]
    )
...
def _draw_mini_track(ax, telemetry: TelemetryData, zones: DrivingZones):
    ...
    for start, end in zones.braking_zones:
        ax.plot(
            x[start : end + 1], y[start : end + 1], color=COLORS["braking"], linewidth=3
        )
...
def _draw_speed_trace(ax, telemetry: TelemetryData, zones: DrivingZones):
    ...
    # Plot speed
    ax.fill_between(
        distance_km, telemetry.speed, alpha=0.3, color=COLORS["full_throttle"]
    )
    ...
    for start, end in zones.braking_zones:
        ax.axvspan(
            distance_km[start], distance_km[end], alpha=0.3, color=COLORS["braking"]
        )
...
def _draw_input_trace(ax, telemetry: TelemetryData):
    ...
    brake_normalized = (
        telemetry.brake * 100 if telemetry.brake.max() <= 1 else telemetry.brake
    )
    ax.fill_between(
        distance_km,
        -brake_normalized,
...
def _draw_gear_trace(ax, telemetry: TelemetryData, zones: DrivingZones):
    ...
    ax.step(
        distance_km, telemetry.gear, where="mid", color=COLORS["coasting"], linewidth=2
    )
    ax.fill_between(
        distance_km, telemetry.gear, step="mid", alpha=0.3, color=COLORS["coasting"]
    )
