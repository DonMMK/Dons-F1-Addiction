...
from data_loader import (
    TelemetryData, DrivingZones, 
    get_lap_telemetry, analyze_driving_zones, get_circuit_info
)
...
DRIVER_TEAMS = {
    ...
    'MAG': 'Haas F1 Team', 'HUL': 'Haas F1 Team', 'BEA': 'Haas F1 Team',
}
...
def interpolate_telemetry_to_distance(telemetry: TelemetryData, 
                                       target_distances: np.ndarray) -> TelemetryData:
    """
    Interpolate telemetry data to common distance points for fair comparison.
    """
    # Create interpolation functions for each channel
    interp_x = interpolate.interp1d(telemetry.distance, telemetry.x, 
                                     kind='linear', fill_value='extrapolate')
    interp_y = interpolate.interp1d(telemetry.distance, telemetry.y, 
                                     kind='linear', fill_value='extrapolate')
    interp_speed = interpolate.interp1d(telemetry.distance, telemetry.speed,
                                         kind='linear', fill_value='extrapolate')
    interp_throttle = interpolate.interp1d(telemetry.distance, telemetry.throttle,
                                            kind='linear', fill_value='extrapolate')
    interp_brake = interpolate.interp1d(telemetry.distance, telemetry.brake,
                                         kind='linear', fill_value='extrapolate')
    interp_gear = interpolate.interp1d(telemetry.distance, telemetry.gear,
                                        kind='nearest', fill_value='extrapolate')
    interp_time = interpolate.interp1d(telemetry.distance, telemetry.time,
                                        kind='linear', fill_value='extrapolate')
    interp_drs = interpolate.interp1d(telemetry.distance, telemetry.drs,
                                       kind='nearest', fill_value='extrapolate')
...
def analyze_lap_comparison(
    ...
    for i in range(num_segments):
        ...
        time_delta = segment_time2 - segment_time1  # Positive = driver1 faster in segment
...
def create_car_icon(ax, x, y, angle=0, size=1.0, color='#FF0000'):
    ...
    car_body = np.array([
        [0, -0.5], [-0.15, -0.45], [-0.18, -0.3], [-0.2, 0],
        [-0.18, 0.3], [-0.1, 0.45], [0, 0.55], [0.1, 0.45],
        [0.18, 0.3], [0.2, 0], [0.18, -0.3], [0.15, -0.45],
    ]) * s
...
class GhostComparisonReplay:
    ...
    def _draw_header(self):
        ...
        self.ax_header.text(0.195, 0.5, comp.driver1, color='white',
                           fontsize=20, fontweight='bold', ha='center', va='center')
        
        # Format lap time
        min1 = int(comp.driver1_time // 60)
        sec1 = comp.driver1_time % 60
        self.ax_header.text(0.195, 0.2, f"{min1}:{sec1:06.3f}", color='white',
                           fontsize=11, ha='center', va='center')
        
        # VS in middle
        self.ax_header.text(0.5, 0.5, "VS", color='#FFD700',
        ...
        self.ax_header.text(0.805, 0.5, comp.driver2, color='white',
                           fontsize=20, fontweight='bold', ha='center', va='center')
        
        min2 = int(comp.driver2_time // 60)
        sec2 = comp.driver2_time % 60
        self.ax_header.text(0.805, 0.2, f"{min2}:{sec2:06.3f}", color='white',
                           fontsize=11, ha='center', va='center')
        ...
        self.ax_header.text(0.5, 0.15, f"{faster} faster by {delta_sign}{abs(delta):.3f}s",
                           color=delta_color, fontsize=10, ha='center', va='center')
    ...
    def _draw_comparison_track(self):
        ...
        self.ax_track.plot(self.x1, self.y1, color='#666666',
                          linewidth=22, alpha=0.3, zorder=1)
        ...
        # Start/finish marker
        self.ax_track.scatter(self.x1[0], self.y1[0], s=300, c='white',
                             marker='s', zorder=5, edgecolors='#00FF00', linewidths=3)
        self.ax_track.text(self.x1[0], self.y1[0], 'S/F', color='#00FF00',
                          fontsize=9, ha='center', va='center', fontweight='bold', zorder=6)
        
        # Title
        self.ax_track.set_title(self.title, color='white', fontsize=14, fontweight='bold', pad=10)
        
        # Legend
        legend_elements = [
            mpatches.Patch(facecolor=self.color1, edgecolor='white', 
                          label=f'{self.comparison.driver1} faster'),
            mpatches.Patch(facecolor=self.color2, edgecolor='white',
                          label=f'{self.comparison.driver2} faster'),
            mpatches.Patch(facecolor='#888888', edgecolor='white', label='Equal'),
        ]
        legend = self.ax_track.legend(handles=legend_elements, loc='upper left',
                                      facecolor='#252540', edgecolor='white',
    ...
    def _draw_delta_panel(self):
        ...
        # Title
        self.ax_delta.text(0.5, 0.95, "GAP ANALYSIS", color='white',
                          fontsize=12, fontweight='bold', ha='center', va='top')
        
        # Current delta display
        self.ax_delta.text(0.5, 0.82, "Current Gap", color='#888888',
                          fontsize=10, ha='center', va='center')
        
        self.delta_text = self.ax_delta.text(0.5, 0.70, "+0.000s",
        ...
        # Driver labels
        self.ax_delta.text(0.15, 0.45, self.comparison.driver1, color=self.color1,
                          fontsize=10, fontweight='bold', ha='center')
        self.ax_delta.text(0.85, 0.45, self.comparison.driver2, color=self.color2,
                          fontsize=10, fontweight='bold', ha='center')
        
        # Mini-sector breakdown title
        self.ax_delta.text(0.5, 0.35, "━━ SECTOR TIMES ━━", color='#888888',
        ...
        for i in range(num_display_sectors):
            y_pos = 0.28 - (i * 0.055)
            txt = self.ax_delta.text(0.5, y_pos, f"S{i+1}: --", color='#888888',
                                    fontsize=9, ha='center', va='center')
    ...
    def _setup_telemetry_display(self):
        ...
        # Plot speed traces
        self.ax_telemetry.plot(dist_km, self.tel1.speed, color=self.color1,
                               linewidth=2, alpha=0.8, label=self.comparison.driver1)
        self.ax_telemetry.plot(dist_km, self.tel2.speed, color=self.color2,
                               linewidth=2, alpha=0.8, label=self.comparison.driver2)
        
        # Current position indicator (will be updated)
        self.pos_line = self.ax_telemetry.axvline(x=0, color='white',
        ...
        self.ax_telemetry.set_ylim(0, max(self.tel1.speed.max(), self.tel2.speed.max()) * 1.05)
        
        legend = self.ax_telemetry.legend(loc='upper right', facecolor='#252540',
    ...
    def _setup_speed_display(self):
        ...
        # Title
        self.ax_speed.text(0.5, 0.95, "SPEED", color='#888888',
                          fontsize=10, ha='center', va='top')
        
        # Driver 1 speed
        self.ax_speed.add_patch(Circle((0.25, 0.65), 0.15, facecolor=self.color1,
                                       edgecolor='white', linewidth=2, alpha=0.3,
                                       transform=self.ax_speed.transAxes))
        self.speed1_text = self.ax_speed.text(0.25, 0.65, "0", color='white',
                                              fontsize=18, fontweight='bold',
                                              ha='center', va='center')
        self.ax_speed.text(0.25, 0.45, self.comparison.driver1, color=self.color1,
                          fontsize=9, fontweight='bold', ha='center')
        
        # Driver 2 speed
        self.ax_speed.add_patch(Circle((0.75, 0.65), 0.15, facecolor=self.color2,
                                       edgecolor='white', linewidth=2, alpha=0.3,
                                       transform=self.ax_speed.transAxes))
        self.speed2_text = self.ax_speed.text(0.75, 0.65, "0", color='white',
                                              fontsize=18, fontweight='bold',
                                              ha='center', va='center')
        self.ax_speed.text(0.75, 0.45, self.comparison.driver2, color=self.color2,
                          fontsize=9, fontweight='bold', ha='center')
        
        # Speed difference
        self.ax_speed.text(0.5, 0.25, "Δ Speed", color='#888888',
                          fontsize=9, ha='center')
        self.speed_diff_text = self.ax_speed.text(0.5, 0.12, "+0 km/h", color='white',
    ...
    def _setup_progress_bar(self):
        ...
        self.progress_bar = self.ax_progress.barh([0.5], [0], height=0.4,
                                                   color='#00AAFF', alpha=0.8)[0]
        
        # Distance markers
        for pct in [0, 25, 50, 75, 100]:
            self.ax_progress.axvline(x=pct, color='#666666', linewidth=0.5, alpha=0.5)
            self.ax_progress.text(pct, 0.1, f"{pct}%", color='#888888', fontsize=8, ha='center')
    
    def _create_dynamic_elements(self):
        """Create car icons and trails."""
        # Driver 1 car
        self.car1_patch = create_car_icon(self.ax_track, self.x1[0], self.y1[0],
                                          self.angles1[0], size=1.0, color=self.color1)
        self.ax_track.add_patch(self.car1_patch)
        
        # Driver 2 car
        self.car2_patch = create_car_icon(self.ax_track, self.x2[0], self.y2[0],
                                          self.angles2[0], size=1.0, color=self.color2)
        self.ax_track.add_patch(self.car2_patch)
        
        # Trails
        self.trail_length = 60
        self.trail1, = self.ax_track.plot([], [], '-', color=self.color1,
                                           linewidth=4, alpha=0.4, zorder=9)
        self.trail2, = self.ax_track.plot([], [], '-', color=self.color2,
                                           linewidth=4, alpha=0.4, zorder=9)
        
        # Driver labels above cars
        self.label1 = self.ax_track.text(self.x1[0], self.y1[0], self.comparison.driver1,
                                         color='white', fontsize=8, fontweight='bold',
                                         ha='center', va='bottom', zorder=20,
                                         bbox=dict(boxstyle='round,pad=0.2',
                                                  facecolor=self.color1, alpha=0.8))
        self.label2 = self.ax_track.text(self.x2[0], self.y2[0], self.comparison.driver2,
                                         color='white', fontsize=8, fontweight='bold',
                                         ha='center', va='bottom', zorder=20,
                                         bbox=dict(boxstyle='round,pad=0.2',
                                                  facecolor=self.color2, alpha=0.8))
    ...
    def _setup_controls(self):
        ...
        self.btn_pause = Button(ax_pause, '⏸ Pause', color='#333333', hovercolor='#888888')
        self.btn_reset = Button(ax_reset, '⏹ Reset', color='#333333', hovercolor='#FF4444')
    ...
    def _update_frame(self, frame_idx: int):
        ...
        self.car1_patch = create_car_icon(self.ax_track, x1, y1,
                                          self.angles1[frame_idx], size=1.0, color=self.color1)
        self.ax_track.add_patch(self.car1_patch)
        
        # Update car 2
        self.car2_patch.remove()
        self.car2_patch = create_car_icon(self.ax_track, x2, y2,
                                          self.angles2[frame_idx], size=1.0, color=self.color2)
        self.ax_track.add_patch(self.car2_patch)
        
        # Update trails
        trail_start = max(0, frame_idx - self.trail_length)
        self.trail1.set_data(self.x1[trail_start:frame_idx+1], self.y1[trail_start:frame_idx+1])
        self.trail2.set_data(self.x2[trail_start:frame_idx+1], self.y2[trail_start:frame_idx+1])
        ...
        for i, sector in enumerate(self.comparison.mini_sectors[:5]):
            if current_dist >= sector['start_distance'] and current_dist < sector['end_distance']:
        ...
        return [self.car1_patch, self.car2_patch, self.trail1, self.trail2,
                self.delta_text, self.speed1_text, self.speed2_text]
...
def create_comparison_summary_plot(
    ...
    # Start/finish
    ax_track.scatter(x[0], y[0], s=200, c='white', marker='s', zorder=5,
                    edgecolors='#00FF00', linewidths=2)
    
    ax_track.set_aspect('equal')
    ax_track.axis('off')
    ax_track.set_title(f"Track Advantage: {comparison.driver1} vs {comparison.driver2}",
                      color='white', fontsize=12, fontweight='bold')
    
    # Legend
    legend_elements = [
        mpatches.Patch(facecolor=comparison.driver1_color, label=f'{comparison.driver1} faster'),
        mpatches.Patch(facecolor=comparison.driver2_color, label=f'{comparison.driver2} faster'),
    ]
    legend = ax_track.legend(handles=legend_elements, loc='upper left',
                             facecolor='#252540', edgecolor='white', fontsize=9)
    plt.setp(legend.get_texts(), color='white')
    ...
    ax_speed.plot(dist_km, tel1.speed, color=comparison.driver1_color, linewidth=2,
                 label=comparison.driver1, alpha=0.9)
    ax_speed.plot(dist_km, tel2.speed, color=comparison.driver2_color, linewidth=2,
                 label=comparison.driver2, alpha=0.9)
    
    # Fill between to show advantage
    ax_speed.fill_between(dist_km, tel1.speed, tel2.speed,
                         where=(tel1.speed > tel2.speed),
                         facecolor=comparison.driver1_color, alpha=0.3)
    ax_speed.fill_between(dist_km, tel1.speed, tel2.speed,
                         where=(tel1.speed < tel2.speed),
                         facecolor=comparison.driver2_color, alpha=0.3)
    ...
    ax_speed.set_title("Speed Comparison", color='white', fontsize=12, fontweight='bold')
    ...
    ax_delta.fill_between(dist_km, delta, 0, where=(delta > 0),
                         facecolor=comparison.driver1_color, alpha=0.5,
                         label=f'{comparison.driver1} ahead')
    ax_delta.fill_between(dist_km, delta, 0, where=(delta < 0),
                         facecolor=comparison.driver2_color, alpha=0.5,
    ...
    ax_delta.set_title("Gap/Delta Over Lap", color='white', fontsize=12, fontweight='bold')
    ...
    colors = [comparison.driver1_color if d > 0 else comparison.driver2_color for d in sector_deltas]
    
    bars = ax_sectors.bar(sector_nums, sector_deltas, color=colors, alpha=0.8, edgecolor='white')
    ...
    ax_sectors.set_title("Mini-Sector Time Deltas", color='white', fontsize=12, fontweight='bold')
    ...
    ax_sectors.text(0.98, 0.98, f"Total: {faster} by {abs(total_ms):.0f}ms",
                   color='white', fontsize=10, fontweight='bold',
                   ha='right', va='top', transform=ax_sectors.transAxes,
