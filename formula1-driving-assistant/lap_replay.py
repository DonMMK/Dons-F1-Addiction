...
class LapReplayAnimation:
    ...
    def _draw_track_conditions(self):
        ...
        if not self.track_conditions:
            self.ax_conditions.text(
                0.5,
                0.5,
                "Track conditions not available",
        ...
        tire_circle = Circle(
            (0.42, 0.5),
            0.03,
            facecolor=tire_color,
            edgecolor="white",
            linewidth=1.5,
            transform=self.ax_conditions.transAxes,
        )
        self.ax_conditions.add_patch(tire_circle)
        self.ax_conditions.text(
            0.46,
            0.5,
            f"{tc.tire_compound} (Lap {tc.tire_life})",
            color=COLORS["text"],
            fontsize=11,
            ha="left",
            va="center",
        ...
        self.ax_conditions.text(
            0.82,
            0.5,
            f"Track: {weather.track_temp:.0f}°C",
            color=COLORS["text_dim"],
            fontsize=10,
            ha="left",
            va="center",
    ...
    def _draw_track(self):
        ...
        self.ax_track.plot(
            self.x, self.y, color=COLORS["track"], linewidth=12, alpha=0.8, zorder=2
        )
        ...
        self.ax_track.set_title(
            self.title, color=COLORS["text"], fontsize=14, fontweight="bold", pad=10
        )
    ...
    def _setup_status_display(self):
        ...
        self.status_main_text = self.ax_status.text(
            0.5,
            0.82,
            "READY",
        ...
        self.gear_change_text = self.ax_status.text(
            0.5,
            0.68,
            "",
        ...
        # Corner name
        self.corner_name_text = self.ax_status.text(
            0.5,
            0.48,
            "---",
            color=COLORS["text"],
        ...
        self.corner_type_text = self.ax_status.text(
            0.5,
            0.40,
            "",
            color=COLORS["text_dim"],
        ...
        # Corner direction
        self.corner_direction_text = self.ax_status.text(
            0.5,
            0.33,
            "",
            color=COLORS["text_dim"],
        ...
        # Corner phase
        self.corner_phase_text = self.ax_status.text(
            0.5,
            0.26,
            "",
            color=COLORS["text_highlight"],
        ...
        # Corner speeds
        self.corner_speeds_text = self.ax_status.text(
            0.5,
            0.17,
            "",
            color=COLORS["text_dim"],
        ...
        # DRS indicator
        self.drs_text = self.ax_status.text(
            0.5,
            0.05,
            "",
    ...
    def _setup_input_bars(self):
        ...
        # Background bars
        self.ax_inputs.barh(
            [2.0], [100], height=0.6, color=COLORS["gauge_bg"], alpha=0.5
        )
        self.ax_inputs.barh(
            [1.0], [100], height=0.6, color=COLORS["gauge_bg"], alpha=0.5
        )
        ...
        self.throttle_pct_text = self.ax_inputs.text(
            102,
            2.0,
            "0%",
            color=COLORS["throttle"],
        ...
        self.brake_pct_text = self.ax_inputs.text(
            102, 1.0, "0%", color=COLORS["brake"], fontsize=10, ha="left", va="center"
        )

        # Dynamic bars (will be updated)
        self.throttle_bar = self.ax_inputs.barh(
            [2.0], [0], height=0.6, color=COLORS["throttle"], alpha=0.8
        )[0]
        self.brake_bar = self.ax_inputs.barh(
            [1.0], [0], height=0.6, color=COLORS["brake"], alpha=0.8
        )[0]
    ...
    def _setup_speed_display(self):
        ...
        # Speed value
        self.speed_text = self.ax_speed.text(
            0.5,
            0.6,
            "0",
            color=COLORS["text"],
        ...
        self.gear_text = self.ax_speed.text(
            0.25,
            0.02,
            "N",
            color=COLORS["text"],
        ...
        self.time_text = self.ax_speed.text(
            0.75,
            0.02,
            "0:00.000",
            color=COLORS["text"],
    ...
    def _setup_progress_bar(self):
        ...
        # Background
        self.ax_progress.barh(
            [0.5], [100], height=0.4, color=COLORS["gauge_bg"], alpha=0.5
        )

        # Progress bar (dynamic)
        self.progress_bar = self.ax_progress.barh(
            [0.5], [0], height=0.4, color=COLORS["speed_bar"], alpha=0.8
        )[0]

        # Distance markers
        for pct in [0, 25, 50, 75, 100]:
            self.ax_progress.axvline(
                x=pct, color=COLORS["text_dim"], linewidth=0.5, alpha=0.3
            )
    ...
    def _create_dynamic_elements(self):
        ...
        self.car_patch = create_car_icon(
            self.ax_track,
            self.x[0],
            self.y[0],
            initial_angle,
        ...
        (self.trail_line,) = self.ax_track.plot(
            [], [], "-", color=COLORS["car_trail"], linewidth=4, alpha=0.5, zorder=9
        )

        # Glow effect around car
        (self.car_glow,) = self.ax_track.plot(
            [], [], "o", color=COLORS["car"], markersize=25, alpha=0.3, zorder=8
        )
    ...
    def _setup_controls(self):
        ...
        self.btn_play = Button(
            ax_play, "> Play", color=COLORS["gauge_bg"], hovercolor=COLORS["throttle"]
        )
        ...
        self.btn_reset = Button(
            ax_reset, "[] Reset", color=COLORS["gauge_bg"], hovercolor=COLORS["brake"]
        )
    ...
    def _update_frame(self, frame_idx: int) -> list:
        ...
        self.car_patch = create_car_icon(
            self.ax_track, car_x, car_y, car_angle, size=1.0, color=COLORS["car"]
        )
        self.ax_track.add_patch(self.car_patch)
        ...
        self.trail_line.set_data(
            self.x[trail_start : frame_idx + 1], self.y[trail_start : frame_idx + 1]
        )
    ...
    def _update_status_display(self, status: dict):
        ...
        if corner:
            ...
            self.corner_direction_text.set_text(
                f"{dir_arrow} {corner.direction} ({corner.angle:.0f} deg)"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Fastest lap: {lap_info.driver} - {lap_info.lap_time}"
        if lap_info
        else "No lap info"
    )
    ...
    if telemetry:
        ...
        for corner in enhanced_corners[:3]:
            print(
                f"  {corner.name}: {corner.corner_type} ({corner.speed_class}) - {corner.direction}"
            )
