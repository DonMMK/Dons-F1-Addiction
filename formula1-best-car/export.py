...
def save_chart_for_tiktok(
    fig: plt.Figure, filename: str, output_dir: Optional[Path] = None
) -> str:
    """
    ...
    fig.savefig(
        output_path, dpi=200, facecolor="#1E1E1E", bbox_inches="tight", pad_inches=0.1
    )
    ...
    with Image.open(image_path) as img:
        ...
        try:
            ...
            font_subtitle = ImageFont.truetype(
                "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", font_size_subtitle
            )
        except:
        ...
        if subtitle:
            ...
            draw.text(
                (subtitle_x, subtitle_y), subtitle, fill="#CCCCCC", font=font_subtitle
            )
...
def create_speed_animation(
    ...
    # Title
    title = ax.set_title(
        f"{driver} - {session.event['EventName']}",
        color="white",
        fontsize=16,
        fontweight="bold",
    )
...
def create_comparison_animation(
    ...
    output_path = output_path or str(
        ANIMATIONS_DIR / f"{driver1}_vs_{driver2}_animation.mp4"
    )
...
def batch_export_race(
    ...
    if export_charts:
        ...
        try:
            ...
            if fig:
                exports["charts"].append(
                    str(CHARTS_DIR / f"{year}_{race_name}_speed_trace.png")
                )
                plt.close(fig)
        ...
        for driver in drivers:
            try:
                fig = create_track_speed_map(
                    ...
                    save_path=str(
                        CHARTS_DIR / f"{year}_{race_name}_{driver}_track_speed.png"
                    ),
                )
                if fig:
                    exports["charts"].append(
                        str(CHARTS_DIR / f"{year}_{race_name}_{driver}_track_speed.png")
                    )
                    plt.close(fig)
            except Exception as e:
        ...
        if len(drivers) >= 2:
            try:
                ...
                if fig:
                    exports["charts"].append(
                        str(CHARTS_DIR / f"{year}_{race_name}_comparison.png")
                    )
                    plt.close(fig)
            except Exception as e:
        ...
        try:
            ...
            if fig:
                exports["charts"].append(
                    str(CHARTS_DIR / f"{year}_{race_name}_sectors.png")
                )
                plt.close(fig)
        except Exception as e:
    ...
    if export_animations and len(drivers) >= 2:
        try:
            path = create_comparison_animation(
                ...
                output_path=str(
                    ANIMATIONS_DIR
                    / f"{year}_{race_name}_{drivers[0]}_vs_{drivers[1]}.mp4"
                ),
            )
...
if __name__ == "__main__":
    ...
    fig = create_speed_trace_comparison(
        session, ["VER", "LEC", "HAM"], title="Test Speed Comparison"
    )
