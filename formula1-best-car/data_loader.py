...
def load_session(
    year: int, race: str | int, session_type: str = "R"
) -> fastf1.core.Session:
    """
...
def get_team_fastest_lap(
    session: fastf1.core.Session, team: str
) -> Optional[fastf1.core.Lap]:
...
def load_season_data(config: ComparisonConfig, session_type: str = "Q") -> Dict:
    ...
    season_data = {"year": config.year, "config": config, "races": {}}
    ...
    for race_name in race_list:
        try:
            ...
            race_data = {"session": session, "primary": {}, "competitors": {}}
