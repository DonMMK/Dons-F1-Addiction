...
def calculate_dominance_metrics(
    year: int, team: str, drivers: List[str], session_type: str = "Q"
) -> DominanceMetrics:
    """
    ...
    for race in races:
        try:
            ...
            try:
                ...
                if len(results) >= 2:
                    if (
                        results.iloc[0]["Abbreviation"] in drivers
                        and results.iloc[1]["Abbreviation"] in drivers
                    ):
                        one_twos += 1
...
def calculate_performance_metrics(
    year: int, team: str, drivers: List[str], races: Optional[List[str]] = None
) -> PerformanceMetrics:
    """
...
def compare_telemetry(
    ...
    comparison = {
        ...
        "brake": {
            label1: np.interp(
                distance_points, tel1["Distance"], tel1["Brake"].astype(float)
            ),
            label2: np.interp(
                distance_points, tel2["Distance"], tel2["Brake"].astype(float)
            ),
        },
        "gear": {
    ...
    comparison["speed_delta"] = (
        comparison["speed"][label1] - comparison["speed"][label2]
    )
...
def analyze_corner_performance(
    ...
    telemetry = (
        telemetry.add_distance() if "Distance" not in telemetry.columns else telemetry
    )
            ...
            pace_data.append(
                {
                    ...
                    "StdLapTime": (
                        lap_times.std().total_seconds() if len(lap_times) > 1 else 0
                    ),
...
def calculate_season_progression(
    year: int, team: str, drivers: List[str]
) -> pd.DataFrame:
    """
    ...
    for era_name, config in configs.items():
        ...
        try:
            dominance = calculate_dominance_metrics(
                config.year, config.primary_car.team, config.primary_car.drivers
            )
...
def get_quali_battle_summary(year: int, drivers: List[str]) -> Dict:
    """
        ...
        try:
            ...
            if lap1 is not None and lap2 is not None:
                time1 = (
                    ...
                    if pd.notna(lap1["LapTime"])
                    else None
                )
                time2 = (
                    lap2["LapTime"].total_seconds()
                    if pd.notna(lap2["LapTime"])
                    else None
                )
