...
from analysis import (
    ...
    compare_eras,
    calculate_season_progression,
    calculate_race_pace_analysis,
    compare_telemetry,
)
from visualizations import (
    ...
    create_dominance_gap_chart,
    create_sector_comparison,
    create_lap_time_distribution,
...
def analyze_season_dominance(
    ...
    results = {"year": year, "team": team, "drivers": drivers, "charts": [], "data": {}}
    ...
    race_list = (
        races
        or schedule[schedule["EventFormat"] != "testing"]["EventName"].tolist()[:5]
    )
    ...
    results["data"]["avg_gap_to_p2"] = (
        sum(gaps_to_p2) / len(gaps_to_p2) if gaps_to_p2 else 0
    )
    ...
    if save_charts:
        ...
        fig = create_dominance_gap_chart(
            ...
            save_path=str(
                CHARTS_DIR / f"{year}_{team.replace(' ', '_')}_dominance.png"
            ),
        )
        if fig:
...
def run_full_analysis():
    ...
    for era_name, config in COMPARISON_CONFIGS.items():
        print(f"\n\n{'#'*60}")
        print(
            f"# ERA: {era_name} - {config.primary_car.team} {config.primary_car.car_name}"
        )
        print("#" * 60)
