...
def create_speed_trace_comparison(
    ...
    for driver in drivers:
        ...
        try:
            style = fastf1.plotting.get_driver_style(
                driver, ["color", "linestyle"], session
            )
        ...
        ax.plot(
            ...
            telemetry["Speed"],
            label=(
                f"{driver} ({lap['LapTime']})" if pd.notna(lap["LapTime"]) else driver
            ),
            color=color,
            linestyle=linestyle,
            linewidth=2,
    ...
    ax.legend(
        loc="lower right", facecolor="#2E2E2E", edgecolor="white", labelcolor="white"
    )
...
def create_track_comparison_map(
    ...
    cbar = plt.colorbar(lc, ax=ax, shrink=0.8)
    cbar.set_label(
        f"← {driver2} faster | {driver1} faster →", color="white", fontsize=12
    )
    cbar.ax.yaxis.set_tick_params(color="white")
    cbar.outline.set_edgecolor("white")
    plt.setp(plt.getp(cbar.ax.axes, "yticklabels"), color="white")
...
def create_dominance_gap_chart(
    ...
    ax.axhline(
        y=avg_gap,
        color=get_team_color(primary_team),
        linestyle="--",
        linewidth=2,
        alpha=0.7,
    )
...
def create_telemetry_comparison_panel(
    ...
    axes[0].legend(
        loc="upper right", facecolor="#2E2E2E", edgecolor="white", labelcolor="white"
    )
...
def create_season_progression_chart(
    ...
    ax.plot(
        progression_data["Round"],
        progression_data["GapToP1"],
        color=color,
        linewidth=2,
        marker="o",
        markersize=6,
