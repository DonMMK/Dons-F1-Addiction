...
class TelemetryAnalyzer:
    ...
    def analyze_stints(self, session):
        ...
        try:
            ...
            rprint(
                "[red]Could not parse laps. Session might be empty or declared Wet.[/red]"
            )
            return {}
    ...
    def run_simulation(self):
        ...
        for _ in range(Config.MONTE_CARLO_RUNS):
            ...
            for grid_pos, driver in enumerate(self.grid):
                ...
                time_s2 = ((base + 0.4) * stint2_len) + (
                    (deg * 0.7) * np.sum(range(stint2_len))
                )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR V3 | {args.year} {args.gp} | {args.session}",
            style="bold red",
        )
    )
    ...
    if args.session in ["FP1", "FP2"]:
        ...
        if not profiles:
            console.print(
                "[red]No valid long runs found. Try a different session.[/red]"
            )
            return
        ...
        if not physics:
            console.print(
                "[red]No Physics data found! Did you run FP1/FP2 first?[/red]"
            )
            return
