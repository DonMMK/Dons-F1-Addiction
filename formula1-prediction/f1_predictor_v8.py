...
class TelemetryAnalyzer:
    ...
    def analyze_stints(self, session):
        ...
        for driver_num, meta in driver_meta.items():
            ...
            for stint_id in d_laps["Stint"].unique():
                ...
                if (
                    ...
                    or final_pace < raw_profiles[driver_abbr]["base_pace"]
                ):
        ...
        if raw_profiles:
            tier1_paces = [
                p["base_pace"] for p in raw_profiles.values() if p["tier"] == 1
            ]
            if tier1_paces:
                ...
                for abbr, data in raw_profiles.items():
                    if data["tier"] == 1:
                        ...
                        if data["base_pace"] > (tier1_median + 0.8):
                            rprint(
                                f"[yellow]⚠ DETECTED SANDBAG/TEST: {abbr} (Tier 1) is slow. Resetting to Tier 1 Median.[/yellow]"
                            )
            ...
            if tier1_paces:
                ...
                for abbr, data in raw_profiles.items():
                    if data["tier"] > 1 and data["base_pace"] < fastest_tier1:
                        rprint(
                            f"[yellow]⚠ GLORY RUN: {abbr} is faster than fastest Tier 1. Clamping.[/yellow]"
                        )
            ...
            if sc_event:
                ...
                for i in range(len(driver_states)):
                    driver_states[i]["total_time"] = (
                        leader_time + (i * 0.6) + np.random.normal(0, 0.5)
                    )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR V8 (TIER LOGIC) | {args.year} {args.gp}", style="bold red"
        )
    )
    ...
    if args.session in ["FP1", "FP2"]:
        ...
        for d, p in sorted(profiles.items(), key=lambda x: x[1]["base_pace"]):
            table.add_row(
                d, p["team"], str(p["tier"]), f"{p['base_pace']:.3f}s", p["status"]
            )
        console.print(table)
