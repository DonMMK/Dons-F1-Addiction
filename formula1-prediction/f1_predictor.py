...
class WeekendState:
    ...
    def load_physics_profile(year, gp):
        ...
        if data["meta"]["year"] != year or data["meta"]["gp"] != gp:
            rprint(
                "[yellow]⚠ Warning: Stored data is for a different race. Ignoring.[/yellow]"
            )
            return None
...
class TelemetryAnalyzer:
    ...
    def load_data(self):
        rprint(
            f"[bold cyan]Loading {self.year} {self.gp} {self.session_type}...[/bold cyan]"
        )
        try:
        ...
        for driver in session.drivers:
            ...
            for stint_id in d_laps["Stint"].unique():
                ...
                stint = stint[
                    ...
                    < median_time * Config.OUTLIER_THRESHOLD
                ]
                ...
                fuel_correction = (
                    Config.RACE_START_FUEL - 60.0
                ) * Config.FUEL_BURN_PER_KG
                corrected_pace = avg_raw_pace + fuel_correction

                compound = (
                    stint["Compound"].iloc[0]
                ...
                if (
                    driver not in driver_profiles
                    or corrected_pace < driver_profiles[driver]["base_pace"]
                ):
...
class RaceSimulator:
    ...
    def run_simulation(self):
        ...
        if not self.physics:
            rprint(
                "[yellow]⚠ No physics data available. Predicting based on Grid only.[/yellow]"
            )
        ...
        for _ in range(Config.MONTE_CARLO_RUNS):
            ...
            for driver in self.grid:
                ...
                # Pit Stop
                pit_time = Config.PIT_LOSS_BASELINE + np.random.normal(
                    0, 0.5
                )  # Random variance
                ...
                total_fuel_gain = (
                    (Config.RACE_START_FUEL * Config.FUEL_BURN_PER_KG) * self.laps * 0.5
                )
                ...
                total_time = (
                    stint1_time + pit_time + stint2_time - total_fuel_gain + variance
                )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR ENGINE | {args.year} {args.gp} | {args.session}",
            style="bold red",
        )
    )
    ...
    if args.session in ["FP1", "FP2", "FP3"]:
        ...
        if not physics:
            console.print(
                "[yellow]⚠ Warning: No FP2/FP3 data found in history. Simulation will be generic.[/yellow]"
            )
        else:
            console.print(
                f"[green]✔ Loaded Physics Profiles for {len(physics)} drivers.[/green]"
            )
