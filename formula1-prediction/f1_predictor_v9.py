...
class TrackConfig:
    ...
    def get_config(gp_name):
        # Default values
        cfg = {"laps": 58, "deg_mult": 1.0, "pass_delta": 0.7}

        # Fuzzy match
        for key in TrackConfig.LAP_COUNTS:
            if key in gp_name:
                cfg["laps"] = TrackConfig.LAP_COUNTS[key]

        for key in TrackConfig.DEG_SENSITIVITY:
            if key in gp_name:
                cfg["deg_mult"] = TrackConfig.DEG_SENSITIVITY[key]

        for key in TrackConfig.OVERTAKE_DELTAS:
            if key in gp_name:
                cfg["pass_delta"] = TrackConfig.OVERTAKE_DELTAS[key]
    ...
    def load_data(self):
        try:
            ...
            if self.session_type == "FP2":
                rprint("[yellow]HINT: Try '--session FP1'.[/yellow]")
            return None
    ...
    def analyze_stints(self, session):
        ...
        for driver_num, driver_abbr in driver_map.items():
            ...
            for stint_id in d_laps["Stint"].unique():
                ...
                # Sanity Checks
                if deg_slope < -0.1:
                    deg_slope = 0.0  # Track evo
                if deg_slope > 0.5:
                    deg_slope = 0.5  # Puncture/Traffic
            ...
            if all_paces:
                ...
                for abbr, data in raw_profiles.items():
                    if data["base_pace"] < threshold:
                        rprint(
                            f"[yellow]⚠ ANOMALY: {abbr} too fast. Clamping.[/yellow]"
                        )
    ...
    def run_simulation(self):
        ...
        for _ in range(Config.MONTE_CARLO_RUNS):
            ...
            for d in driver_states:
                ...
                deg_loss = np.sum(
                    [
                        d["deg"] * lap * self.cfg["deg_mult"]
                        for lap in range(self.cfg["laps"])
                    ]
                )
            ...
            if sc_event:
                ...
                for i in range(len(driver_states)):
                    # Compressed field
                    driver_states[i]["total_time"] = (
                        leader_time + (i * 0.5) + np.random.normal(0, 0.5)
                    )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR V9 (DEGRADATION ENGINE) | {args.year} {args.gp}",
            style="bold red",
        )
    )
    ...
    if args.session in ["FP1", "FP2"]:
        ...
        for d, p in sorted(profiles.items(), key=lambda x: x[1]["base_pace"]):
            table.add_row(
                d, f"{p['base_pace']:.3f}s", f"{p['deg_slope']:.4f}", p["status"]
            )
        console.print(table)
