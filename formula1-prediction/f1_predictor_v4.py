    ...
    def load_data(self):
        try:
            ...
            if self.session_type == "FP2":
                rprint(
                    "[yellow]HINT: This might be a Sprint Weekend. Try using '--session FP1' instead.[/yellow]"
                )
            return None
    ...
    def analyze_stints(self, session):
        ...
        for driver_num in session.drivers:
            ...
            for stint_id in d_laps["Stint"].unique():
                ...
                if (
                    driver_abbr not in driver_profiles
                    or corrected_pace < driver_profiles[driver_abbr]["base_pace"]
                ):
...
class RaceSimulator:
    def __init__(self, physics_profile, grid_order, gp_name):
        ...
        rprint(
            f"[bold yellow]Track Difficulty: {gp_name} (Delta required: {self.overtake_delta}s/lap)[/bold yellow]"
        )
    ...
    def run_simulation(self):
        ...
        for _ in range(Config.MONTE_CARLO_RUNS):
            ...
            for grid_pos, driver in enumerate(self.grid):
                ...
                race_time = (
                    (base * self.laps)
                    + (deg * np.sum(range(self.laps)))
                    + np.random.normal(0, 4.0)
                )

                # Add Grid Position index to track order
                driver_times.append(
                    {"driver": driver, "raw_time": race_time, "grid_pos": grid_pos}
                )
            ...
            for i in range(1, len(driver_times)):
                ...
                time_diff = (
                    prev_driver_time - curr_raw_time
                )  # Positive means Current is faster
                ...
                prev_driver_time = (
                    curr_raw_time  # This car becomes the blockage for the next one
                )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR V4 (PHYSICS + TRACK LOGIC) | {args.year} {args.gp}",
            style="bold red",
        )
    )
