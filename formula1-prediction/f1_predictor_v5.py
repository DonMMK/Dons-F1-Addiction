...
class Config:
    ...
    SC_CHANCE = {"Monaco": 0.6, "Singapore": 1.0, "Italy": 0.4, "DEFAULT": 0.3}
...
class TrackConfig:
    ...
    def get_overtake_delta(gp_name):
        return TrackConfig.OVERTAKE_DELTAS.get(
            gp_name, TrackConfig.OVERTAKE_DELTAS["DEFAULT"]
        )
...
class TelemetryAnalyzer:
    ...
    def load_data(self):
        try:
            ...
            if self.session_type == "FP2":
                rprint(
                    "[yellow]HINT: This might be a Sprint Weekend. Try using '--session FP1'.[/yellow]"
                )
            return None
    ...
    def analyze_stints(self, session):
        ...
        raw_profiles = {}
        driver_map = {
            d: session.get_driver(d)["Abbreviation"]
        ...
        for driver_num, driver_abbr in driver_map.items():
            ...
            for stint_id in d_laps["Stint"].unique():
                ...
                reg = LinearRegression().fit(
                    np.arange(len(times)).reshape(-1, 1), times
                )
                ...
                corrected_pace = avg_raw + (
                    (Config.RACE_START_FUEL - 60.0) * Config.FUEL_BURN_PER_KG
                )

                # Keep best stint
                if (
                    driver_abbr not in raw_profiles
                    or corrected_pace < raw_profiles[driver_abbr]["base_pace"]
                ):
        ...
        if raw_profiles:
            ...
            for abbr, data in raw_profiles.items():
                if data["base_pace"] < (median_field_pace - 1.2):
                    rprint(
                        f"[yellow]⚠ ANOMALY DETECTED: {abbr} is {-1*(data['base_pace'] - median_field_pace):.2f}s faster than field median.[/yellow]"
                    )
                    rprint(
                        f"[yellow]  -> Reclassifying as Low Fuel Run. Clamping to valid Top 3 pace.[/yellow]"
                    )
    ...
    def run_simulation(self):
        ...
        for _ in range(Config.MONTE_CARLO_RUNS):
            ...
            for d in driver_states:
                # Basic Race Time
                d["total_time"] += (d["pace"] * self.laps) + (
                    d["deg"] * np.sum(range(self.laps))
                )
...
def main():
    ...
    console.print(
        Panel.fit(
            f"F1 PREDICTOR V5 (ANOMALY DETECTION) | {args.year} {args.gp}",
            style="bold red",
        )
    )
