...
def analyze_driving_zones(
    ...
    coasting = (telemetry.throttle < throttle_partial_threshold) & (
        telemetry.brake < brake_threshold
    )
...
def _find_corners(
    telemetry: TelemetryData, min_points: int = 5
) -> List[Dict[str, Any]]:
    """
    ...
    for i in range(min_points, len(speed_smooth) - min_points):
        ...
        for j in range(1, min_points + 1):
            if (
                speed_smooth[i] >= speed_smooth[i - j]
                or speed_smooth[i] >= speed_smooth[i + j]
            ):
                is_minimum = False
                break
...
def get_circuit_info(session) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "rotation": (
                float(circuit.rotation) if hasattr(circuit, "rotation") else 0.0
            ),
            "corners": (
                circuit.corners.to_dict("records")
...
def get_track_conditions(
    session, driver: Optional[str] = None
) -> Optional[TrackConditions]:
    """
    ...
    try:
        ...
        compound = str(fastest.get("Compound", "UNKNOWN"))
        tire_life = (
            int(fastest.get("TyreLife", 0)) if pd.notna(fastest.get("TyreLife")) else 0
        )
...
def get_enhanced_corners(
    session, telemetry: TelemetryData, corner_zones: List[Dict[str, Any]]
) -> List[CornerInfo]:
    """
    ...
    for i, corner in enumerate(corner_zones):
        ...
        direction, turn_angle = get_corner_direction(
            telemetry.x[before_idx],
            telemetry.y[before_idx],
            telemetry.x[apex_idx],
            telemetry.y[apex_idx],
            telemetry.x[after_idx],
            telemetry.y[after_idx],
        )
...
if __name__ == "__main__":
    ...
    for event in schedule[:3]:
        print(
            f"  Round {event['round_number']}: {event['event_name']} ({event['circuit_name']})"
        )
